...
$Id: SynEdit.pas,v 1.386.2.73 2008/06/02 04:26:15 maelh Exp $
...
2008/07/16
- new procedures:
  - MyDelete (delete symbol at right from cursor);
  - MyShiftTab (as Shift+Tab combination);
  - MyTab (as Tab button);
  - AddToClipboard (add selected text to clipboard);
  - ChangeClipboard (change selected text with text from clipboard);
...
{$ENDIF}
	...
	//the procedures below added by Aleksey Tatuyko are Copyright (C) 2008 Aleksey Tatuyko. 
	//procedures added at 2008/07/16
    procedure MyDelete;
    procedure MyShiftTab;
    procedure MyTab;
	//this is end of new procedures block - 2008/06/01
    ...
    //this is new procedures below
	//the procedures below added by Aleksey Tatuyko are Copyright (C) 2008 Aleksey Tatuyko. 
	//procedures added at 2008/06/01
...
//the procedures below added by Aleksey Tatuyko are Copyright (C) 2008 Aleksey Tatuyko. 
//procedures added at 2008/06/01
procedure TCustomSynEdit.AddToClipboard;
var
  SText: WideString;
  ChangeTrim: Boolean;
begin
  ...
  begin
    ...
    DoCopyToClipboard(GetClipboardText+SText);
  end;
end;

procedure TCustomSynEdit.ChangeClipboard;
var
  SText, Temp: WideString;
  ChangeTrim: Boolean;
begin
  if SelAvail then
  begin
...
procedure TCustomSynEdit.ClearClipboard;
begin
  Clipboard.Open;
  Clipboard.Clear;
  Clipboard.Close;
...
procedure TCustomSynEdit.SetLines(Value: TWideStrings);
begin
  Lines.Assign(Value);
end;

procedure TCustomSynEdit.SetLineText(Value: WideString);
begin
...
procedure TCustomSynEdit.SetSelTextPrimitive(const Value: WideString);
begin
...
procedure TCustomSynEdit.SynSetText(const Value: WideString);
begin
  Lines.Text := Value;
end;
...
var
  ...
  Point: TPoint;
  FilesList: TWideStringList;
begin
  try
    if Assigned(fOnDropFiles) then
    begin
      FilesList := TWideStringList.Create;
      try
...
begin
  ...
  begin
    StrLCopyW(PWideChar(Msg.Text), PWideChar(Text), Msg.TextMax - 1);
    Msg.Result := StrLenW(PWideChar(Msg.Text));
  end
  else
  begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          PWLength := MultiByteToWideChar(DefaultUserCodePage, 0, PA, ImeCount,
            nil, 0);
          GetMem(PW, (PWLength + 1) * sizeof(WideChar));
          try
            MultiByteToWideChar(DefaultUserCodePage, 0, PA, ImeCount,
              PW, PWLength);
            CommandProcessor(ecImeStr, #0, PW);
          finally
            FreeMem(PW);
...
procedure TCustomSynEdit.MyTab;
begin
  IncPaintLock;
  try
...
procedure TCustomSynEdit.MyShiftTab;
begin
  IncPaintLock;
  try
...
procedure TCustomSynEdit.MyDelete;
...
begin
  ...
  try
    if not ReadOnly then begin
      DoOnPaintTransient(ttBefore);
      if SelAvail then SetSelectedTextEmpty
      else begin
        ...
        if (Caret.Char <> CaretX) or (Caret.Line <> CaretY) then begin
          fUndoList.AddChange(crSilentDeleteAfterCursor, CaretXY, Caret, Helper, smNormal);
        end;
      end;
        DoOnPaintTransient(ttAfter);
      end;  
...
// Processing of case character covers on LeadByte.
      ...
      ecImeStr:
        if not ReadOnly then
        begin
          SetString(S, PWideChar(Data), StrLenW(Data));
          if SelAvail then
          begin
            BeginUndoBlock;
            try
...
procedure TCustomSynEdit.SetSelTextExternal(const Value: WideString);
var
...
begin
  ...
  begin
    ...
    while (first_nonblank <= vMaxX) and
      (s[first_nonblank] in [WideChar(#32), WideChar(#9)])
    do
      inc(first_nonblank);
    dec(first_nonblank);
...
begin
  ...
  begin
    ...
    if WordWrap then
      ...
      vMinX := 0;
    while (vLastNonBlank > vMinX) and (vText[vLastNonBlank] in [WideChar(#32), WideChar(#9)]) do
      Dec(vLastNonBlank);
...
begin
  ...
  try
    ...
    begin
      ...
      StrToInsert := StrAllocW(InsertStrLen);
      Run := StrToInsert;
    ...
    begin
      StrCopyW(Run, PWideChar(Spaces + #13#10));
      Inc(Run, Length(spaces) + 2);
    end;
    StrCopyW(Run, PWideChar(Spaces));
...
begin
  ...
  begin
    ...
    //                chars per line * lines-1    + last line + null char
    StrToDelete := StrAllocW(StrToDeleteLen);
    StrToDelete[0] := #0;
    ...
    begin
       ...
       TmpDelLen := GetDelLen;
       StrCatW(StrToDelete, PWideChar(Copy(Line, 1, TmpDelLen)));
       StrCatW(StrToDelete, PWideChar(WideString(#13#10)));
    ...
    if FullStrToDelete <> nil then
      StrDisposeW(FullStrToDelete)
    else
      StrDisposeW(StrToDelete);
  end;
end;
...
function TCustomSynEdit.DoOnReplaceText(const ASearch, AReplace: WideString;
...
function TCustomSynEdit.GetWordAtRowCol(XY: TBufferCoord): WideString;
var
  Line: WideString;
  Len, Stop: Integer;
begin
...
procedure TCustomSynEdit.ProperSetLine(ALine: Integer; const ALineText: WideString);
begin
  if eoTrimTrailingSpaces in Options then
...
function TCustomSynEdit.GetWordAtMouse: WideString;
var
  Point: TBufferCoord;
begin

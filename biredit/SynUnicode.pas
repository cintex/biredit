...
$Id: SynUnicode.pas,v 1.1.2.38.10 2008/08/23 08:49:00 maelh Exp $

You may retrieve the latest version of this file at the SynEdit home page,
located at http://SynEdit.SourceForge.net


Changes in code by Aleksey Tatuyko:

2008/08/24 1.1.2.38.10
- some changes in TWideStrings.LoadFromFile and TWideStrings.SaveToFile procedures
- deleted TWideStrings.MyLoadFromStream, TWideStrings.MyLoadFromFile and TWideStrings.MyNotSaved procedures

2008/08/11 1.1.2.38.5
- EFOpenError exception processing in TWideStrings.MyLoadFromFile procedure

2008/08/07 1.1.2.38.4
- EOutOfMemory exception processing in TWideStrings.GetTextStr function

2008/06/01 1.1.2.38.3
- new procedures:
  - TWideStrings.MyLoadFromFile procedure
  - TWideStrings.MyLoadFromStream procedure
  - TWideStrings.MyNotSaved procedure
...
- An adapted and lighter version of TWideStrings/TWideStringList taken
...
{ TWideStrings }
  ...
  //this is modified function below
	//the function below modified by Aleksey Tatuyko
	//function modified at 2008/08/07
    function GetTextStr: WideString; virtual;
	//this is end of modified function block - 2008/08/07
  ...
  //this is new procedures below
	//the procedures below modified by Aleksey Tatuyko
  //procedures modified at 2008/08/24
    procedure LoadFromFile(const FileName: string); virtual;
	//this is end of modified procedures block - 2008/08/24
    procedure LoadFromStream(Stream: TStream); virtual;
    procedure Move(CurIndex, NewIndex: Integer); virtual;
  //this is new procedures below
	//the procedures below modified by Aleksey Tatuyko
  //procedures modified at 2008/08/23
    procedure SaveToFile(const FileName: string); virtual;
	//this is end of modified procedures block - 2008/08/23
...
function StrMoveW(Dest: PWideChar; const Source: PWideChar; Count: Integer): PWideChar;
...
{$IFDEF SYN_WIN32}
var
  DefaultUserCodePage: Cardinal; // implicitly used when converting AnsiString <--> WideString.
  
function CharUpperW(lpsz: PWideChar): PWideChar;
function CharUpperBuffW(lpsz: PWideChar; cchLength: DWORD): DWORD;
function CharLowerW(lpsz: PWideChar): PWideChar;
function CharLowerBuffW(lpsz: PWideChar; cchLength: DWORD): DWORD;
{$ENDIF}
function SynWideUpperCase(const S: WideString): WideString;
function SynWideLowerCase(const S: WideString): WideString;
...
function TextExtent(ACanvas: TCanvas; const Text: WideString): TSize;
function TextWidth(ACanvas: TCanvas; const Text: WideString): Integer;
function TextHeight(ACanvas: TCanvas; const Text: WideString): Integer;
procedure TextOut(ACanvas: TCanvas; X, Y: Integer; const Text: WideString);
...
function WideFileCreate(const FileName: WideString): Integer; overload;
...
uses
  //
  Unit1, BirEditAdv, TntDialogs, Dialogs,
  //
  {$IFDEF SYN_CLX}
  QSynEditTextBuffer,
  {$ELSE}
...
begin
  ...
  try Result := GetSeparatedText(WideCRLF) except on E : EOutOfMemory do WideMessageDlg(Editor.errm1, mtError, [mbOK], 0) end;
end;
//this is end of modified function block - 2008/08/07
...
//this is modified procedure below
//the procedure modified added by Aleksey Tatuyko
//procedure modified at 2008/08/24
...
var
 ...
 begin
   ...
   Editor.MyFileName:=FileName;
   Editor.myfsize:=Stream.Size;
 end;

begin
  try Stream := TFileStream.Create(FileName, fmOpenRead or fmShareDenyWrite) except on E : EFOpenError do WideMessageDlg(Editor.errm2, mtError, [mbOK], 0) end;
  try
    if Stream.Size>Editor.bigsize then begin
      case WideMessageDlg(Editor.mysg4,mtWarning,[mbYes,mbNo],0) of
        mrYes: MyLoad;
      end;
    end else MyLoad;
  finally
...
//this is end of modified procedure block - 2008/08/24
...
//this is modified procedure below
//the procedure below modified by Aleksey Tatuyko
//procedure modified at 2008/08/23
...
//this is end of modified procedure block - 2008/08/23
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$IFDEF SYN_CLX}
        lpsz^ := WideChar(QSynUnicode.CharUpperW(PWideChar(lpsz^)));
        {$ELSE}
        lpsz^ := WideChar(SynUnicode.CharUpperW(PWideChar(lpsz^)));
        {$ENDIF}
        Inc(lpsz);
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$IFDEF SYN_CLX}
        lpsz^ := WideChar(QSynUnicode.CharLowerW(PWideChar(lpsz^)));
        {$ELSE}
        lpsz^ := WideChar(SynUnicode.CharLowerW(PWideChar(lpsz^)));
        {$ENDIF}
        Inc(lpsz);
      end;
    end;
...
begin
  ...
  begin
    if Win32PlatformIsUnicode then
      ...
      Result := Integer(CreateFile(PChar(AnsiString(FileName)), AccessMode[Mode and 3],
        ShareMode[(Mode and $F0) shr 4], nil, OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL, 0));
  end;
end;
...
begin
  if Win32PlatformIsUnicode then
    ...
    Result := Integer(CreateFile(PChar(AnsiString(FileName)), GENERIC_READ or GENERIC_WRITE,
      0, nil, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0));
end;
{$ENDIF}
{$IFDEF SYN_LINUX}
...
function WideFileCreate(const FileName: WideString; Rights: Integer): Integer;
{$IFDEF SYN_WIN32}
begin
...
{$IFDEF SYN_WIN32}
begin
  Result := IsWideStringMappableToAnsi(WS);
end;
{$ELSE}
var
  Run: PWideChar;
...
begin
  Stream := TWideFileStream.Create(FileName, fmCreate);
  try
    SaveToStream(WideStrings, Stream, Encoding, WithBom);
  finally
    Stream.Free;
  end;
end;
...
procedure SaveToStream(const WS: WideString; Stream: TStream; Encoding: TSynEncoding;
...
begin
  ...
  case Encoding of
    ...
    seAnsi:
      begin
        AnsiStr := WideCharToString(PWideChar(WS));
...
begin
  // if WideStrings or Stream is nil, let Delphi raise the exception to
  // signal an invalid result
  WideStrings.BeginUpdate;
  try
    Result := Encoding;
    ...
    case Result of
      seUTF8:
        begin
          ...
          WideStrings.Text := UTF8Decode(UTF8Str);
          WideStrings.SaveUnicode := True;
        end;
      seUTF16LE:
        begin
          SetLength(WideStr, Size div 2);
          Stream.ReadBuffer(WideStr[1], Size);
          WideStrings.Text := WideStr;
          WideStrings.SaveUnicode := True;
        end;
      seUTF16BE:
        begin
      ...
      seAnsi:
        begin
          ...
          WideStrings.Text := AnsiStr;
          WideStrings.SaveUnicode := False;
        end;
    end;
  finally
    WideStrings.EndUpdate
  end
end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        begin
          P := GlobalLock(Mem);
          Result := StringToWideStringEx(PAnsiChar(P), CodePageFromLocale(LocaleID));
        end
      finally
        if Mem <> 0 then GlobalUnlock(Mem);
      end;
...
type
  TWideStringPropertyFiler = class
  private
...
{ TWideStringPropertyFiler }

procedure TWideStringPropertyFiler.DefineProperties(Filer: TFiler; Instance: TPersistent;
  PropName: AnsiString);
...
procedure TWideStringPropertyFiler.ReadData(Reader: TReader);
begin
  case Reader.NextValue of
    vaLString, vaString:
...
procedure TWideStringPropertyFiler.WriteData(Writer: TWriter);
begin
...
var
  ...
  PropInfo: PPropInfo;
  PropList: PPropList;
  WideStringFiler: TWideStringPropertyFiler;
  WideCharFiler: TWideCharPropertyFiler;
begin
  Count := GetTypeData(Instance.ClassInfo)^.PropCount;
  if Count > 0 then
  begin
    WideStringFiler := TWideStringPropertyFiler.Create;
    try
...
begin
  WideCharToMultiByte(DefaultUserCodePage, 0, PWideChar(@WC), 1, nil, 0, nil,
    @UsedDefaultChar);
...
var
  UsedDefaultChar: BOOL;
begin
  WideCharToMultiByte(DefaultUserCodePage, 0, PWideChar(WS), Length(WS), nil, 0,
    nil, @UsedDefaultChar);
  Result := not UsedDefaultChar;
end;
{$ENDIF}
